...
class ModuleAPIHandler extends APIHandler
{
    ...
    public function getZCRMField($fieldDetails)
    {
		...
		// Modified beacause mandatory was not being set correctly
        $fieldInstance->setMandatory(
			( isset($fieldDetails['required']) ? (boolean) $fieldDetails['required'] : false )
			|| ( isset($fieldDetails['system_mandatory']) ? (boolean) $fieldDetails['system_mandatory'] : false )
		);
