...
class MeprStripeGateway extends MeprBaseRealGateway {
  ...
  /**
   * For subscriptions, set up the grace period confirmation transaction (if enabled)
   *
   * @param MeprTransaction  $txn
   * @param MeprSubscription $sub
   */
  public function handle_create_subscription(MeprTransaction $txn, MeprSubscription $sub) {
    $mepr_options = MeprOptions::fetch();
  ...
  public function process_create_subscription($txn) {
    ...
    $this->handle_create_subscription($txn, $sub);
  }
  ...
  public function process_resume_subscription($sub_id) {
    ...
    $args = MeprHooks::apply_filters('mepr_stripe_resume_subscription_args', array(
      ...
      'off_session' => 'true',
      'expand' => array('latest_invoice.payment_intent.payment_method')
    ), $sub);
    ...
    $subscr = (object) $this->send_stripe_request( "customers/{$sub->subscr_id}/subscriptions", $args, 'post' );

    $invoice = (object) $subscr->latest_invoice;

    if ($subscr->latest_invoice['status'] == 'open' && $invoice->payment_intent) {
  ...
  */
  ...
  public function send_latest_invoice_payment_email(MeprSubscription $sub) {
    ...
    $subscr = (object) $this->send_stripe_request( "customers/{$customer->id}/subscription", $args, 'get' );
