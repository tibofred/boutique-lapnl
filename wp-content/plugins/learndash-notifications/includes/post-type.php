...
function learndash_notifications_post_type_custom_columns( $column, $post_id ) {
	switch ( $column ) {
		...
		case 'topic':
			...
			if ( in_array( $trigger, array( 'enroll_group', 'enroll_course', 'upload_assignment', 'approve_assignment', 'essay_graded', 'not_logged_in', 'complete_lesson', 'lesson_available', 'course_expires' ) ) ) {
				echo '-';
				break;
			}

			if ( $topic_id == 'all' || empty( $topic_id ) || ! is_numeric( $topic_id ) ) {
