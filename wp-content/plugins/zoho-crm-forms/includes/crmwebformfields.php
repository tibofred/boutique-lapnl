                ...
                <div>
                    <div>
                        ...
                        <div id="fieldtable">
                            ...
                            if (isset($shortcode))
                                echo $FieldOperations->zcfformFields("zcf_crmfields_shortcodes", sanitize_text_field($_REQUEST['onAction']), $shortcode, 'post', sanitize_text_field($_REQUEST['module']), sanitize_text_field($_REQUEST['LayoutName']));
                            else
                                echo $FieldOperations->zcfformFields("zcf_crmfields_shortcodes", sanitize_text_field($_REQUEST['onAction']), '', 'post', sanitize_text_field($_REQUEST['onAction']), sanitize_text_field($_REQUEST['module']), sanitize_text_field($_REQUEST['LayoutName']));
                            ?>
                        </div>
                        <!-- </div> -->
                    </div>
        ...
        <div class="pp-content pb10">
            ...
            <div>
                <div class="form-group col-md-12">
                    <?php
                    $resultaiss = $wpdb->get_results("select * from zcf_zohocrm_assignmentrule where modulename='" . sanitize_text_field($_REQUEST['module']) . "'");
                    ?>
                    <div id='innertext' class="col-md-4">
                        <label class="leads-builder-label">
                            ...
                            $resultmodule = $wpdb->get_results("select * from zcf_zohocrm_assignmentrule where modulename='" . sanitize_text_field($_REQUEST['module']) . "'");
                    ...
                    <?php
                    $resultlayout = $wpdb->get_results("select sm.assignmentrule_ID from zcf_zohocrm_assignmentrule ar join zcf_zohoshortcode_manager sm where sm.assignmentrule_ID=ar.assignmentrule_ID and sm.shortcode_name='" . sanitize_text_field($_REQUEST['EditShortcode']) . "'");
...
<div class="newPopup ppTop dB" id="create-fields-list-popup" role="dialog">
    ...
    <div class="pp-content">
        <?php
        $shortcode_array = $wpdb->get_row("select shortcode_id from zcf_zohoshortcode_manager where shortcode_name='" . sanitize_text_field($_REQUEST['EditShortcode']) . "'");
