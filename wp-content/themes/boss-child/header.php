...
<html <?php language_attributes(); ?>>
	...
	<head>
		...
		<script async src="https://www.googletagmanager.com/gtag/js?id=UA-52739114-1"></script>
		<script>
		  window.dataLayer = window.dataLayer || [];
		  function gtag(){dataLayer.push(arguments);}
		  gtag('js', new Date());

		  gtag('config', 'UA-52739114-1');
		</script>
		<?php
			$page_url = $_SERVER['REQUEST_URI'];
			if(!strpos($page_url, 'courses') && !strpos($page_url, 'lessons')) {
				?>
		<style type="text/css">
			#primary {
				width: 100%;
...
//    $boxed	 = 'fluid';
	...
	<body <?php body_class(); ?> data-logo="<?php echo $logo; ?>" data-inputs="<?php echo $inputs; ?>" data-rtl="<?php echo ($rtl) ? 'true' : 'false'; ?>" data-header="<?php echo $header_style; ?>">
		...
		if ( is_user_logged_in() ) {
			...
			$afficher_formation = get_field('afficher_formation', 'user_'.$u_id);
			if(!empty($afficher_formation[0]) && $afficher_formation[0] == 1) {
				$class_css = "form_showing";
			}

		}
		?>
		<div id="panels" class="<?php echo (boss_get_option( 'boss_adminbar' )) ? 'with-adminbar' : ''; ?> <?php echo $class_css;?>">
